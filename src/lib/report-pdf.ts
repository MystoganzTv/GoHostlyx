...
export async function generateShareReportPdf(input: ShareReportPdfInput) {
  ...
  drawLabel(page, mono, "GoHostlyx Financial Summary", contentX, contentTop - 2, rgb(0.44, 0.5, 0.61));
  ...
  const footerText = input.view.monthlySummary[0]?.label && input.view.monthlySummary.at(-1)?.label
    ? `Coverage: ${input.view.monthlySummary[0].label} to ${input.view.monthlySummary.at(-1)?.label}. Generated by GoHostlyx.`
    : "Generated by GoHostlyx from imported bookings and expenses.";
